...
public class ImageTargetRenderer implements GLSurfaceView.Renderer, SampleAppRendererControl {
    ...
    public ImageTargetRenderer(ImageTargets activity, SampleApplicationSession session, onDetected onDetected) {
        mActivity = activity;
        vuforiaAppSession = session;
        // SampleAppRenderer used to encapsulate the use of RenderingPrimitives setting
        // the device mode AR/VR and stereo mode
...
//                            mTextures.get(3).mTextureID[0]);
    ...
    private void printUserData(Trackable trackable) {
        ...
        if ((!isNotified)) {
            isNotified = true;
            System.out.println("yoyoyoyo..1..");
            muserData = userData;
            onDetected.showUI(muserData);
        }

        if (muserData != null && (muserData != userData)) {
            isNotified = false;
            if (!isNotified) {
                System.out.println("yoyoyoyo..2..");
                onDetected.showUI(userData);
            }
        }
    }
